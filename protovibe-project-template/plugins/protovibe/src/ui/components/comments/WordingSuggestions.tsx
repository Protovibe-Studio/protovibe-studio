...
//     while rows exist: original → suggested fields, live-previewing each change
//     on the canvas as the writer types. Fully controlled by the parent's rows —
//     it has no state of its own, so it can never linger after submit/cancel.
 ...
 * never on submit, where the preview intentionally stays active.
 */
export function clearSuggestionPreviews(rows: WordingSuggestion[]): void {
  const preview = getCopySuggestionPreview();
  for (const row of rows) preview.remove(row.original);
}
 ...
 * seeds one row per text string extracted from the selected element, which makes
 ...
 * holds one row per extracted string (original + current suggested); the parent
 * owns it so it can fold the changed rows onto the comment on submit and reset
 * on cancel. Renders nothing when there are no rows — visibility is entirely
 * driven by the toggle button seeding/clearing the rows.
 */
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: theme.text_secondary, fontSize: 11, fontWeight: 600, fontFamily: theme.font_ui }}>
        ...
        <button
          type="button"
          onClick={() => { clearSuggestionPreviews(value); onChange([]); }}
 ...
 * derived live from the preview service and compares the SUGGESTED value too —
 * when two comments propose different copy for the same original string, only
 * the one whose exact suggestion is on the canvas reads as active, and turning
 * one on transparently supersedes the other.
 */
export const SuggestionPreviewBlock: React.FC<{
  ...
  const previewing = useSyncExternalStore(
    preview.subscribe,
    () => suggestions.length > 0 && suggestions.every((s) => preview.get(s.original) === s.suggested),
  );
  ...
  const toggle = () => {
    ...
    // never clears a different comment's preview that took over the original.
    if (previewing) suggestions.forEach((s) => preview.remove(s.original, s.suggested));
