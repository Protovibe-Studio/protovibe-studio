// plugins/protovibe/src/ui/hooks/useViewportAnchorIds.ts
// Which anchored ids (comment threads, spec annotations) have their element on
// screen right now — backs the "In view" filter of the Comments and Specs panels.
...
/**
 * Set of `candidateIds` whose anchored element (found with `selectorFor`) is
 * currently visible (on screen and not occluded) on the `activeIframeTab`
 * surface. Fully inert unless `enabled`. `selectorFor` must be stable (a
 * module-level function) — it is not an effect dependency.
 *
 ...
 */
export function useViewportAnchorIds(
  enabled: boolean,
  ...
  // Reduce to a stable primitive key so the effect re-runs when the candidate
  // set changes, not on every new array identity.
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      for (const id of candidateIds) {
        const el = doc.querySelector(selectorFor(id));
    ...
    // candidateIds is captured via the stable candidateKey dep; selectorFor is stable.
